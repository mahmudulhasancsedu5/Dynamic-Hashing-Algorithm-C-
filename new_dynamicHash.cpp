...
{
    if(currentLength==bit_length)
    {
        cout << currentLength << endl;
        cout << "Address table can not be extended." << endl;
        //cout << "insertion of " << input << " is not possible." << endl;
        hashNode->bucket.push_back(input);
        return ;
    }
    Node *exp = new Node;
    Node *exp1=new Node;
...
void dynamicHash::display()
{
    ...
    for(exp = root; exp!=NULL; exp = exp->next){
        cout << "address_table entity  " << exp->val << "  ->  ";
